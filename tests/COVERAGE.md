...
| Spec   | Bullet                                                                     | Test file                                       | Status  | Blocked by |
|--------|----------------------------------------------------------------------------|-------------------------------------------------|---------|------------|
| `[S06]`| read config file *or* command-line parameters (booleans coerced via `coerce_bool()` so a CLI `--flag false` matches a config `flag = false`) | `tests/functions/coerce_bool.nf.test`, `tests/main.nf.test`, `tests/processes/part_b/global_clustering.nf.test`, `tests/processes/part_b/cleaving.nf.test` | done | required-param branch covered by `[S18]` |
