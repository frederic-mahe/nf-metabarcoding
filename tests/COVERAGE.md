...
| `[S49]`| Part C stampa primary path — splitFasta + per-chunk `vsearch --usearch_global` + `bin/stampa_merge.py` + `collectFile(sort:)` | `tests/processes/part_c/assign_taxonomy_stampa.nf.test`, `tests/main.nf.test`, `tests/python/test_stampa_merge.py`, `tests/python/test_stampa_merge_legacy.py` | done | — |
