...
| `[S07]`| runs locally or on HPC (slurm)                                             | `tests/check-slurm-config.sh` (profile wiring: executor + resourceLimits + closures resolve, composes with conda, job arrays off by default); execution = manual cluster smoke test | done    | per-process resource tiers, dataset-scaled memory, retry escalation, `submitRateLimit` 50/1min, opt-in `--slurm_array_size` |
